...
const localMockBaseUrl =
  typeof window !== "undefined" && ["5173", "4173", "8081"].includes(window.location.port)
...
const configuredApiBaseUrl = [
  ...
  .find(Boolean);
// Production PWA: same-origin /api via Nginx when env is blank; never fall back to :4000 on the public site.
const baseURL = shouldUseLocalProxy
  ? undefined
  : (configuredApiBaseUrl ?? localMockBaseUrl ?? (typeof window !== "undefined" ? "" : undefined));
